...
const NewRendezVousForm = ({ onSubmit }) => {
  ...
  const validate = () => {
    ...
    if (!formData.patientEmail) newErrors.patientEmail = t("patientEmailRequired");
    // Optionnel : validation simple email
    else if (!/\S+@\S+\.\S+/.test(formData.patientEmail)) newErrors.patientEmail = t("invalidEmail");
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Tu peux aussi récupérer un message d’erreur plus précis si disponible
  ...
  return (
    ...
    >
      ...
      <div>
        <label className="block mb-1">{t("patientEmail")}</label>
        <input
          type="email"
          name="patientEmail"
...
export default NewRendezVousForm;
